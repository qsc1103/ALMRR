...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=64, patch_size=4, stride=4, in_chans=3, embed_dim=768, norm_layer=None, flatten=True):
        super().__init__()
...
def vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=4, embed_dim=192, depth=8, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='all', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=False, if_devide_out=True, use_middle_cls_token=False, **kwargs)
...
def vim_tiny_patch16_stride8_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, stride=8, embed_dim=192, depth=8, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
...
class DecoderDiscriminative(nn.Module):
    ...
    def forward(self, b1,b2,b3,b4,b5,b6):
        up_b = self.up_b(b6) #8
        cat_b = torch.cat((up_b,b5),dim=1) #16
        db_b = self.db_b(cat_b) #8

        up1 = self.up1(db_b) #4
        cat1 = torch.cat((up1,b4),dim=1) #12
        db1 = self.db1(cat1) #4

        up2 = self.up2(db1) #2
        cat2 = torch.cat((up2,b3),dim=1) #6
        db2 = self.db2(cat2) #2

        up3 = self.up3(db2) #1
        cat3 = torch.cat((up3,b2),dim=1) #3
        db3 = self.db3(cat3) #1

        up4 = self.up4(db3) #1
        cat4 = torch.cat((up4,b1),dim=1) #2
        db4 = self.db4(cat4) #1
...
class Decoder_r(nn.Module):
    def __init__(self):
        ...
        self.dec_block1 = nn.Sequential(
            nn.ConvTranspose2d(192, 256, (3, 3)),
        ...
        self.dec_block3 = nn.Sequential(
            nn.ConvTranspose2d(512, 1024, (3, 3), stride=2, padding=1),
            nn.InstanceNorm2d(1024),
            nn.ReLU(True)
        )

        self.up = nn.UpsamplingBilinear2d((64, 64))  # fixed output size
        self.tanh = nn.Tanh()
...
class MFRM(nn.Module):
    def __init__(self, in_channels = 1024, latent_dim = 512):
        super(MFRM, self).__init__()
        self.encoder_rec = vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2()
        self.decoder_rec = Decoder_r()
        self.conv1x1_down = nn.Conv2d(in_channels=in_channels, out_channels=latent_dim, kernel_size=1)
        # 定义升维的1x1卷积层
        self.conv1x1_up = nn.Conv2d(in_channels=latent_dim, out_channels=in_channels, kernel_size=1)
        
        
    def forward(self, x):
        x = self.conv1x1_down(x)
        x = self.encoder_rec(x)
        B,N,C = x.shape
        sqrt_N = int(N ** 0.5)
        x = x.view(B, C, sqrt_N, sqrt_N)
        x = self.decoder_rec(x)
        x = self.conv1x1_up(x)
        return x
        
    def relative_euclidean_distance(self, a, b):
        return (a - b).norm(2, dim=1) / a.norm(2, dim=1)    #计算相对欧氏距离

    def loss_function(self, x, x_hat):
        loss = torch.mean((x - x_hat) ** 2)     #计算均方差损失
        return loss
